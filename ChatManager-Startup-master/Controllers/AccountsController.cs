...
namespace ChatManager.Controllers
{
    public class AccountsController : Controller
    {
        ...
        {
            List<Chat> chats = DB.Chat.ToList().Where(u => u.IdReceive == userid || u.IdSend == userid).ToList();
            List<FriendShip> friendships = DB.FriendShips.ToList().Where(f => f.IdAmis1 == userid || f.IdAmis2 == userid).ToList();
        ...
        [OnlineUsers.AdminAccess]
        public ActionResult AdminMod(int id)
        {
            User editedUser = DB.Users.FindUser(id);
            ViewBag.UserType = SelectListUtilities<UserType>.Convert(DB.UserTypes.ToList());
            return View(editedUser);
        }
        [HttpPost]
        [OnlineUsers.AdminAccess]
        public ActionResult AdminMod(User user)
        {
            User usermod = DB.Users.Get(user.Id);
            user.Avatar = usermod.Avatar;
            string newEmail = "";
            if (ModelState.IsValid)
            {
